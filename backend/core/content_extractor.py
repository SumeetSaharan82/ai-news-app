...
class ContentExtractor:
    ...
    def _extract_title(self, soup: BeautifulSoup, domain: str) -> str:
        ...
        for selector in self.GENERIC_SELECTORS["title"]:
            ...
            if element:
                ...
                if len(text) > 10:  # Filter out empty or too-short titles
                    return text
    ...
    def _extract_body(self, soup: BeautifulSoup, domain: str) -> str:
        ...
        # Try source-specific body selector
        body_element = None
        if domain in self.SOURCE_SELECTORS:
    ...
    def _calculate_confidence(self, title: str, body: str, word_count: int) -> float:
        ...
        # Bonus for sufficient body length
        if word_count > 300:
            confidence += 0.15
        elif word_count > 100:
            confidence += 0.05
...
async def extract_article_content(url: str) -> Optional[ExtractedContent]:
    """
